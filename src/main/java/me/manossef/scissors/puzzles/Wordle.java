...
public class Wordle extends Puzzle {
    ...
    static {
        Resources.loadWords("wordle_words.txt", WORDS, "Failed to initialize the list of words for Wordle games. All " + Commands.defaultFormat("wordle") + " commands will fail during this session.");
        Resources.loadWords("wordle_answers.txt", ANSWERS, "Failed to initialize the list of answers for Wordle games. All " + Commands.defaultFormat("wordle") + " commands will fail during this session.");
    }
    ...
    public void start() {
        ...
        this.getChannel().sendMessage(MessageCreateData.fromEmbeds(new MessageEmbed(null, "Wordle" + (this.hardMode ? " (hard mode)" : ""),
            ...
            null, 0x5865F2, null, null, null, null, null, null, null, 0))).queue();
    }
